...
class GoogleScraperServiceTest {
    ...
    @BeforeEach
    void setUp() {
        googleScraperService = new GoogleScraperService(environment, siteAvailabilityService);
    }

    @Test
    void getPlatform_ShouldReturnGoogleJobs() {
        // When
        Platform platform = googleScraperService.getPlatform();

        // Then
        assertThat(platform).isEqualTo(Platform.GOOGLE_JOBS);
    }
    ...
    void isAvailable_WhenSiteIsReachable_ShouldReturnTrue() {
        ...
        // When
        boolean result = googleScraperService.isAvailable();
    ...
    void scrapeJobs_InDevProfile_WithSingleKeywordAndLocation_ShouldReturnThreeJobs() {
        ...
        // When
        List<Job> jobs = googleScraperService.scrapeJobs(searchCriteria);

        // Then
        assertThat(jobs).hasSize(3);
        assertThat(jobs).allMatch(job -> job.getPlatform() == Platform.GOOGLE_JOBS);
        ...
        assertThat(jobs).allMatch(job -> job.getExternalId().startsWith("Google Jobs-"));
        assertThat(jobs).allMatch(job -> job.getUrl().startsWith(Platform.GOOGLE_JOBS.getUrl()));
    ...
    void scrapeJobs_InDevProfile_ShouldSetCorrectJobProperties() {
        ...
        for (int i = 0; i < jobs.size(); i++) {
            ...
            assertThat(job.getDescription())
                    .contains(keyword)
                    .contains(location);
            assertThat(job.getPlatform()).isEqualTo(Platform.GOOGLE_JOBS);
            assertThat(job.getExternalId()).startsWith("Google Jobs-");
            assertThat(job.getUrl()).startsWith(Platform.GOOGLE_JOBS.getUrl());
        }
    }
