...
class LinkedInScraperServiceTest {
    ...
    @BeforeEach
    void setUp() {
        linkedInScraperService = new LinkedInScraperService(environment, siteAvailabilityService);
    }

    @Test
    void getPlatform_ShouldReturnLinkedIn() {
        // When
        Platform platform = linkedInScraperService.getPlatform();

        // Then
        assertThat(platform).isEqualTo(Platform.LINKEDIN);
    }
    ...
    void isAvailable_WhenSiteIsReachable_ShouldReturnTrue() {
        ...
        // When
        boolean result = linkedInScraperService.isAvailable();

        // Then
        assertThat(result).isTrue();
        verify(siteAvailabilityService).isSiteReachable(Platform.LINKEDIN.getUrl());
    }
    ...
    void scrapeJobs_InDevProfile_WithSingleKeywordAndLocation_ShouldReturnThreeJobs() {
        ...
        // When
        List<Job> jobs = linkedInScraperService.scrapeJobs(searchCriteria);

        // Then
        assertThat(jobs).hasSize(3);
        assertThat(jobs).allMatch(job -> job.getPlatform() == Platform.LINKEDIN);
        ...
        assertThat(jobs).allMatch(job -> job.getExternalId().startsWith("LinkedIn-"));
        assertThat(jobs).allMatch(job -> job.getUrl().startsWith(Platform.LINKEDIN.getUrl()));
    ...
    void scrapeJobs_InDevProfile_ShouldSetCorrectJobProperties() {
        ...
        for (int i = 0; i < jobs.size(); i++) {
            ...
            assertThat(job.getDescription())
                    .contains(keyword)
                    .contains(location);
            assertThat(job.getPlatform()).isEqualTo(Platform.LINKEDIN);
            assertThat(job.getExternalId()).startsWith("LinkedIn-");
            assertThat(job.getUrl()).startsWith(Platform.LINKEDIN.getUrl());
        }
    }
